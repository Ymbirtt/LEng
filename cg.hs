  ...
  where (rNode,regs) = ralloc' n [1..]
        ...
        ralloc' (ADDR c1 c2 c3) regs = (ADDR c1' c2' c3',regs4)
         where (c1',regs1) = ralloc' c1 regs
               ...
               regs4 = deRalloc c3' (deRalloc c2' regs3)
        ...
        ralloc' (BEQZR n s) regs = (BEQZR n' s,regs2)
         where (n',regs1) = ralloc' n regs
               regs2 = deRalloc n' regs1
        ralloc' (BNEZR n s) regs = (BNEZR n' s,regs2)
         where (n',regs1) = ralloc' n regs
               regs2 = deRalloc n' regs1
        ralloc' (BGEZR n s) regs = (BGEZR n' s,regs2)
         where (n',regs1) = ralloc' n regs
               regs2 = deRalloc n' regs1
        ralloc' (BLTZR n s) regs = (BLTZR n' s,regs2)
         where (n',regs1) = ralloc' n regs
               regs2 = deRalloc n' regs1
